			...
			worksheet.getRows(function(err, rows) {
				...
				rows.forEach(function(row, i) {
					...
					if (row.action != 'INVALID' && row.action != 'SHIPPED') {
						// Create new address
						createNewAddress(row).then(function(error) {
							logError(error, i + 2);
							row.action = 'INVALID';

							updateTimestamp(row, function(stamped_row) {
								stamped_row.save();
							});
							return;
						},
...
function createNewShipment(worksheet, address, row) {
	return new Promise(function(reject, resolve) {
		...
		var pack_info = convertPackageInfo(row);
...
function convertSheetAddress(row) {
	var residential = '';
...
function convertPackageInfo(row) {
...
function updateTimestamp(row, callback) {
	var date = new Date();
